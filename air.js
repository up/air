...
/*jslint undef: false, browser: true, nomen: true, regexp: true, indent: 2 */
  ...
  var air = {
    ...
    loadImages: function (config, resize, noscripts) {
      ...
      var _display, res, dot, extension, path, img, nos, $nos, parent, img_tag,
        attrs, attr_parts, attr_name, attr_value, attr_apos, prev_sib,
      ...
      for (res in config) {
        if (config.hasOwnProperty(res) && iWidth >= parseInt(res, 10)) {
          _display = config[res];
        }
      }
      ...
      if (air.checkImages) {
        for (i = 0; i < air._images.length; i += 1) {
          path = air._images[i][0];
          extension = air._images[i][1];
          for (res in config) {
            if (config.hasOwnProperty(res)) {
    ...
    addResizeHandler: function (config) {
